...
for($i = 1; $i <= 45; $i++){
	...
	$query = 'INSERT INTO Player(TeamNo, Fname, Lname, Hometown, Position) VALUES(' . intval(($i-1)/5+1) . ', \'' . str_replace("'", "\\'", $fname) .  '\', \'' . str_replace("'", "\\'", $lname) . '\', \'' . str_replace("'", "\\'", $college) . '\', \'' .str_replace("'", "\\'", $position).'\');';
	echo $query;
	try{
		$a = $sql->query($query);
		echo "<p>$a</p>";
